...
for opt in "${options[@]}"
do
    in6out6+="$input_path/test_$opt.root "
done
for opt in "${options[@]}"
do
    in6out6+="$output_path/test_$opt.root "
done
...
h_output_file="$output_path/test_bbbar.root"
combine="$output_path/test_signal.root $output_path/test_charged.root"
