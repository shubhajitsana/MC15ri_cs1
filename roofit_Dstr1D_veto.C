...
void roofit_Dstr1D_veto(){
    /*******************Fit Variables***********************************/
    RooRealVar deltae("deltae","Invariant mass (GeV)", 2.008, 2.02);
    ...
    chain->Add("/home/belle2/ssana/MC15ri/data/combined/m_cc.root/tree");
    ...
    chain->SetBranchAddress("InvMD0and1stpi",&de3);
    ...
    /*****************************Delta E Fit***********************/
    // --- Build Signal PDF ---
    RooRealVar mean1("mean1","mean of Gaussian-1",2.01026, 2.009, 2.011);
    RooRealVar mean2("mean2","mean of Gaussian-2",2.01026, 2.009, 2.011);
    ...
    RooRealVar fsig_1("frac_gaussians", "signal fraction", 0.77,0.,1.);
    ...
    RooPlot* deframe = deltae.frame(Title("Fitting Invariant mass of 1^{st} pion and #bar{D}^{0}"), Bins(150)) ;                          
    ...
    deframe->Draw() ;
    // Adding legend
    TLegend *legend1 = new TLegend(0.45,0.7,0.62,0.9);
    ...
    entry->SetLineStyle(kDashed);
    entry = legend1->AddEntry("bkg","bkg-Chbyshv","l");
    ...
    c1->Print("roofit_plot_veto/Dstr1D_md.png");
}