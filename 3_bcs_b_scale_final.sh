...
pyrt_file="$path/my_scale_final.py"    # To take data like 50% as signal and 50% as continnum event
...
for opt in "${options[@]}"
do
    in5out5+="$input_path/train_deltaz_filtered_$opt.root "
done
for opt in "${options[@]}"
do
    in5out5+="$output_path/train_deltaz_filtered_$opt.root "
done
