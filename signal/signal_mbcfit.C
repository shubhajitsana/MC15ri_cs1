...
void signal_mbcfit(){
    /*******************Fit Variables***********************************/
    RooRealVar mbc("mbc","M_{bc} (GeV)",5.23,5.3);
    ...
    /*****************************Delta E Fit***********************/
    // --- Build Signal PDF ---
    RooRealVar mean1("mean1","mean of Gaussian-1",5.279145,5.27,5.30);
    RooRealVar mean2("mean2","mean of Gaussian-2",5.270910,5.27,5.30);
    RooRealVar sigma1("sigma1","sigma of Gaussian-1",0.00263109, 0., 10);	
    RooRealVar sigma2("sigma2","sigma of Gaussian-2",0.00911693,0.00000001,10);

    RooGaussian sig1("sig1","Gaussian-1",mbc,mean1,sigma1);  
    RooGaussian sig2("sig2","Gaussian-2",mbc,mean2,sigma2);

    RooRealVar fsig_1("frac_gaussians", "signal fraction", 0.85,0.,1.);
    ...
    RooPlot* deframe = mbc.frame(Title("Fitting M_{bc} of B^{#pm} of signal event"), Bins(200)) ;                          
    ...
    c1->Print("signal_mbc/mbc_fit_3.png");
