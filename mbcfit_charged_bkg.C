...
void mbcfit_charged_bkg(){
  ...
  RooRealVar fsig_1("frac_gaussians", "signal fraction", 0.2,0.,1.);
  ...
  RooRealVar fbkg_1("frac_argus", "argus fraction", 0.13,0.,1.);
  RooAddPdf sum("sum","sum",RooArgList(twoGaussians,bkg),RooArgList(fbkg_1));//adding two pdf
  sum.fitTo(*data);
  /****************************FIT COMPLETE*************************************/
  ...
  entry->SetLineStyle(kDashed);
  entry = legend1->AddEntry("bkg","ARGUS","l");
  ...
  c1->Print("mbc_plot/mbc_fit_charged_bkg.png");
