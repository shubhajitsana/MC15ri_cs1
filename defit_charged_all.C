...
void defit_charged_all(){
    ...
    RooPlot* deframe_charged = deltae_charged.frame(Title("Fitting #DeltaE of B^{#plus}B^{#minus}"), Bins(300)) ;                          
    ...
    entry_charged->SetLineStyle(kDashed);
    entry_charged = legend1_charged->AddEntry("sum_charged","Fitted pdf","l");
    entry_charged->SetLineColor(kBlue);
    entry_charged->SetLineStyle(kSolid);
    legend1_charged->Draw();

    // // Adding arrow at (+-)3*sigma of signal pdf
    // double weightedMean = mean1_charged.getVal()*fsig_1_charged.getVal() + mean2_charged.getVal()*(1.0-fsig_1_charged.getVal());
    // double weightedSigma = std::sqrt( pow(sigma1_charged.getVal(),2)*pow(fsig_1_charged.getVal(),2)  +  pow(sigma2_charged.getVal(),2)*pow((1-fsig_1_charged.getVal()),2));
    // double deltae_min_fit = weightedMean -3*weightedSigma; 
    // double deltae_max_fit = weightedMean +3*weightedSigma;
    // double arrowHeight = n_sig_charged.getVal()*0.03;
    ...
    c1->Print("de_plot/de_fit_charged_all.png");
}