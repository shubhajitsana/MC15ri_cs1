void root_plot_de_before_fitting(){
    ...
    TH1F *mbca = new TH1F("mbca","mbca", 100, -0.1, 0.1);
    TH1F *mbcch = new TH1F("mbcch","mbcch", 100, -0.1, 0.1);
    TH1F *mbcch_signal = new TH1F("mbcch_signal","mbcch_signal", 100, -0.1, 0.1);
    TH1F *mbcch_bkg = new TH1F("mbcch_bkg","mbcch_bkg", 100, -0.1, 0.1);
    TH1F *mbcm = new TH1F("mbcm","mbcm", 100, -0.1, 0.1);
    TH1F *mbcu = new TH1F("mbcu","mbcu", 100, -0.1, 0.1);

    // Filling the histogram
    for(Int_t iEvent=0; iEvent < ta->GetEntries(); iEvent++){
        ta->GetEntry(iEvent);
        if(md_a>1.85 && md_a<1.88 && mbc_a>5.23 && mbc_a < 5.29 && de_a < 0.1 && de_a > -0.1 && kid_a > 0.6 && cont_prob_a < 0.6) mbca->Fill(de_a);
    }
    for(Int_t iEvent=0; iEvent < tch->GetEntries(); iEvent++){
        tch->GetEntry(iEvent);
        if(md_ch>1.85 && md_ch<1.88 && mbc_ch>5.23 && mbc_ch < 5.29 && de_ch < 0.1 && de_ch > -0.1 && kid_ch > 0.6 && cont_prob_ch < 0.6) mbcch->Fill(de_ch);
    }
    for(Int_t iEvent=0; iEvent < tch_signal->GetEntries(); iEvent++){
        tch_signal->GetEntry(iEvent);
        if(md_ch_signal>1.85 && md_ch_signal<1.88 && mbc_ch_signal>5.23 && mbc_ch_signal < 5.29 && de_ch_signal < 0.1 && de_ch_signal > -0.1 && kid_ch_signal > 0.6 && cont_prob_ch_signal < 0.6) mbcch_signal->Fill(de_ch_signal);
    }
    for(Int_t iEvent=0; iEvent < tch_bkg->GetEntries(); iEvent++){
        tch_bkg->GetEntry(iEvent);
        if(md_ch_bkg>1.85 && md_ch_bkg<1.88 && mbc_ch_bkg>5.23 && mbc_ch_bkg < 5.29 && de_ch_bkg < 0.1 && de_ch_bkg > -0.1 && kid_ch_bkg > 0.6 && cont_prob_ch_bkg < 0.6) mbcch_bkg->Fill(de_ch_bkg);
    }
    for(Int_t iEvent=0; iEvent < tm->GetEntries(); iEvent++){
        tm->GetEntry(iEvent);
        if(md_m>1.85 && md_m<1.88 && mbc_m>5.23 && mbc_m < 5.29 && de_m < 0.1 && de_m > -0.1 && kid_m > 0.6 && cont_prob_m < 0.6) mbcm->Fill(de_m);
    }
    for(Int_t iEvent=0; iEvent < tu->GetEntries(); iEvent++){
        tu->GetEntry(iEvent);
        if(md_u>1.85 && md_u<1.88 && mbc_u>5.23 && mbc_u < 5.29 && de_u < 0.1 && de_u > -0.1 && kid_u > 0.6 && cont_prob_u < 0.6) mbcu->Fill(de_u);
    }

    /////////////////////Drawing///////////////////
    TCanvas *c = new TCanvas();

    mbca->SetTitle("Plotting #Delta E");
    mbca->GetYaxis()->SetTitle("Number of Events");
    mbca->GetXaxis()->SetTitle("#Delta E (GeV/c^{2})");
    ...
    c->Print("de_plot/histo_de_scaled_signal_with_cut_ultrazoom.png");
}