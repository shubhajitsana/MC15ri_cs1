...
void roofit_D_10_veto(){
    /*******************Fit Variables***********************************/
    RooRealVar deltae("deltae","#DeltaM (GeV)", 0.4, 0.7);
    ...
    chain->Add("/home/belle2/ssana/MC15ri/data/combined/charged_signal.root/tree");
    ...
    chain->SetBranchAddress("D_10D_md",&de3);
    ...
    /*****************************Delta E Fit***********************/
    // --- Build Signal PDF ---
    RooRealVar mean1("mean1","mean of Gaussian-1",0.556,0.53,0.58);
    RooRealVar mean2("mean2","mean of Gaussian-2",0.557,0.53,0.58);
    ...
    RooRealVar fsig_1("frac_gaussians", "signal fraction", 0.31,0.,1.);
    RooAddPdf twoGaussians("twoGaussians", "sum of two Gaussians ",RooArgList(sig1, sig2), RooArgList(fsig_1));

    // --- Build Argus background PDF ---
    RooRealVar b1("Chbyshv-prm", "Chbyshv-prm", 0.46, -10., 10.);
    ...
    RooPlot* deframe = deltae.frame(Title("Fitting difference of mass of #bar{D}_{1}^{0} and #bar{D}^{0}"), Bins(150)) ;                          
    ...
    c1->Print("roofit_plot_veto/D_10D_md.png");
}